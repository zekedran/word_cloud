...
class WordCloud(object):
    ...
    def save_svg(self, filename):
        with open(filename, 'w+') as f:
            font = ImageFont.truetype(self.font_path, self.max_font_size if self.max_font_size is not None else self.height)
            ...
            print('<svg xmlns="http://www.w3.org/2000/svg" '
                       'width="{0}" height="{1}" style={2}>'.format(
                           self.width, self.height, quoteattr(font_style)), file=f)
            print('\t<rect width="{0}" height="{1}" '
                       'style="fill: {2}"></rect>'.format(self.width, self.height, self.background_color), file=f)
            for (word, count), font_size, position, orientation, color in self.layout_:
                font = ImageFont.truetype(self.font_path, font_size)
